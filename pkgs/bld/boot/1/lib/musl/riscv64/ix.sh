...
{% block step_setup %}
export MFLAGS=
export MFLAGS="${MFLAGS} -iquote ${PWD}/arch/riscv64"
export MFLAGS="${MFLAGS} -iquote ${PWD}/src/internal"
export MFLAGS="${MFLAGS} -isystem ${PWD}/arch/riscv64"
...
{% block patch %}
cat << EOF > arch/riscv64/bits/alltypes.h
{% include 'alltypes.h' %}
EOF
cat << EOF > arch/riscv64/bits/syscall.h
{% include 'syscall.h' %}
EOF
