...
{% block step_setup %}
export MFLAGS=
export MFLAGS="${MFLAGS} -iquote ${PWD}/arch/aarch64"
export MFLAGS="${MFLAGS} -iquote ${PWD}/src/internal"
export MFLAGS="${MFLAGS} -isystem ${PWD}/arch/aarch64"
...
{% block patch %}
cat << EOF > arch/aarch64/bits/alltypes.h
{% include 'alltypes.h' %}
EOF
cat << EOF > arch/aarch64/bits/syscall.h
{% include 'syscall.h' %}
EOF
...
for s in crt/aarch64/*.s crt/crt1.c; do
    cc $s -c -o $s.o
    objs="$s.o $objs"
done
